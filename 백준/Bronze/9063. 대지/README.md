...
모든 점이 포함되면서 최대의 사각형 넓이를 구하는 문제다.
최대, 최소의 x,y 를 파악하여 높이와 넓이를 구하면 된다.
...
```
N을 입력으로 받는다
x좌표와 y좌표를 넣을 리스트를 생성한다

N번동안:
좌표를 받고 리스트에 append 한다

하지만 N이 1이라면
    성립이 되지 않으므로 0을 출력한다
    
그게 아니라면
    x좌표의 min, max로 넓이를 구하고
    y좌표의 min, max로 높이를 구하여
    넓이를 출력한다.
```
...
### 문제 해결 및 배운 점
---
평소 변수명을 신경안썼는데 정확하게 표기하는 연습을 해야겠다
